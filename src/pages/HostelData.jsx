...
const HostelData = () => {
    ...
    const fetchData = () => {
        api.get(`/api/get-hostel-data/${hostelId}/`)
            .then(res => {
    ...
    return (
        <div className="flex flex-col gap-5 px-20 py-10 font-quicksand">
            <div className="flex items-center">
                <BackBtn />
                <h2 className="text-3xl text-primary-700 font-700 ml-5">{hostel.name} Room Allocation Data </h2>
            </div>
            {blocks.map(block => (
                <div key={block.id} className="mb-10">
                    <h2 className="text-2xl text-zinc-600 font-500 mb-3">{block.name}</h2>
                    {rooms.map(room => (
